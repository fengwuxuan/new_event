...
if __name__ == "__main__":
	...
	foler_idx = 0
	interval = 4
	prefix = "/DATACENTER2/ji.zhang/gs_test"
	#output_dir = "/DATACENTER2/ji.zhang/gs_test"
	folder= os.listdir(prefix)
	
	for f in folder:
		folder_dir = os.path.join(prefix, f)
		videos = os.listdir(folder_dir)

		for video in videos:
			if not video.endswith('.mp4'):
				continue
			video_path = os.path.join(folder_dir, video)

			det_dir = video_path[:-4] + '_m_det'
			event_dir = video_path[:-4] + '_m_event'
			#event_dir = os.path.join(output_dir, video[:-4] + '_i_event')
			...
			event_dir = os.path.join(prefix, "event_video")
			if not os.path.exists(event_dir):
				os.mkdir(event_dir)
			out_video = os.path.join(event_dir, video[:-4] + '.avi')
			
			if video.find("fuyong") != -1:
				frame_size = (1280,720)
			else:
				frame_size=(1280, 720)
			...
			while True:
				...
				#cv2.imwrite(os.path.join(event_dir, str(name_idx) + ".jpg"), result_img)
				...
				if name_idx > 1:
					...
					for i, bboxes in enumerate(predict_bboxes):
						...
						#print os.path.join(pics, str(name_idx - interval + i + 1) + '.jpg')

						#writer.write(img)

				# event_json = os.path.join(event_dir, str(name_idx) + ".json")

				# uuids, states, boxes, counts, directions, up_cnt, down_cnt = tracker.get_vehiclesinfo(last_img.shape[0], last_img.shape[1])
				...
				# #result_img = cv2.resize(result_img, out_video_size)
